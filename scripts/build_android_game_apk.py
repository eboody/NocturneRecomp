...
def main() -> None:
    ...
    # Generate sources with the host SDK. The Android SDK's rexglue binary is
    # arm64/bionic and cannot run on this Linux host, so codegen uses the Linux
    # host SDK while compilation/linking uses the Android SDK.
    manifests = sorted(ROOT.glob("*_manifest.toml"))
    if len(manifests) != 1:
        raise SystemExit(f"error: expected one *_manifest.toml, found {manifests}")
    run([host_rexglue, "codegen", manifests[0]], cwd=ROOT)
